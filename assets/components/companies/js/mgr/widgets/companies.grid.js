Companies.grid.Items = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'companies-grid-items';
	}
	Ext.applyIf(config, {
	...
	Companies.grid.Items.superclass.constructor.call(this, config);
...
Ext.extend(Companies.grid.Items, MODx.grid.Grid, {
	windows: {},
	...
	getFields: function (config) {
		return ['id', 'name', 'user_id','customer','description', 'requisites', 'status', 'actions'];
	},
	...
	getColumns: function (config) {
		return [{
			...
			header: _('company_requisites'),
			dataIndex: 'requisites',
			sortable: false,
			width: 200,
		}, {
			header: _('company_status'),
			dataIndex: 'status',
			renderer: Companies.utils.renderBoolean,
			sortable: true,
			width: 50,
		}, {
...
Ext.reg('companies-grid-items', Companies.grid.Items);